...
async function approveMax(signer, spender, tokenAddress, gasPrice) { 
  ...
  cobra.log.debug(`token allowance,owner ${signer.address},spender ${spender},contractAddr ${tokenAddress},allowance ${ethers.utils.formatUnits(allowAmount, "ether")}`)
  const enoughAmount = ethers.utils.parseEther(Number.MAX_SAFE_INTEGER.toString())
  if (allowAmount.lt(enoughAmount.div(10))) {
    cobra.log.debug(`token approve start,address ${signer.address},spender ${spender}`)
    ...
    cobra.log.debug(`token approve wait ,owner ${signer.address},spender ${spender},wait txHash at ${tx.hash},nonce ${tx.nonce},gasPrice ${gasGwei} gwei`)
    await tx.wait()
    cobra.log.debug(`token approve complete,owner ${signer.address},spender ${spender},txHash at ${tx.hash}`)
  }  
}
